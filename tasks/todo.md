...
- [ ] Test copy 4 end-to-end (manual exec or test webhook with a real campaignFolderId; keep INACTIVE — webhook path collides with the active original); verify leads sheet has real on-niche companies, spot-check domains. NOTE: test folder `1O7xJuy4GBolwUUDpz7O2j9wqgcUtzBTm` already contains a `leads` sheet → after the fix the run will correctly stop with SKIP_HAS_LEADS; point it at a folder without a leads file to exercise the HUNT path.
