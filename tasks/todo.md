...
- [ ] **USER: paste into `erp-server/.env`**: the `N8N_*_WEBHOOK_URL`s + `N8N_API_URL`/`N8N_API_KEY` + `ARSENAL_INGEST_TOKEN` + `ANTHROPIC_API_KEY` (Render dashboard → evertrust-api → Environment). Blank = those features stay off. Then `docker compose restart erp-api`.
- [ ] CUTOVER (short window, on go signal): suspend Render API → final re-dump → drop/recreate `evertrust` → restore → restart erp-api → re-smoke → merge `migrate-evertrust-erp` to main + push → remove cutover banners.
