...
### 2026-06-22 - Supabase session pooler: bound the app pool or deploys EMAXCONNSESSION
- **Trigger:** Render deploy crash-looped on `db:migrate` with `PostgresError (EMAXCONNSESSION): max clients reached in session mode - max clients are limited to pool_size: 15`. Render keeps the OLD container (holding its DB pool) alive until the NEW one is healthy; with postgres.js defaulting to `max: 10`, the live instance plus the new instance's migration blew past Supabase's session-pooler 15-client cap, so migrations couldn't get a connection and the deploy exited 1.
- **Lesson:** `packages/db/src/client.ts` opened `postgres(DATABASE_URL)` with NO `max`, so each instance grabbed up to 10 of the 15 shared session-pooler slots. A capped pooler + an always-on container + overlapping deploys = guaranteed contention.
- **Rule going forward:** Against a capped pooler (Supabase session pooler = 15), always bound the app pool (`DATABASE_POOL_MAX`, default 5) and set `idle_timeout` so connections release. Run migrations on a separate connection (`MIGRATION_DATABASE_URL` → Supabase DIRECT) so DDL never competes for pooler slots, and retry `db:migrate` with backoff in `api-start.sh` to ride out the deploy-overlap window. For more capacity, switch `DATABASE_URL` to the TRANSACTION pooler (6543) — but then prepared statements MUST be off (the client auto-disables them for :6543/pgbouncer=true, or set `DATABASE_PREPARE=false`).

### 2026-06-16 - Removing a shared export: verify EVERY consumer package, not just one
- **Trigger:** After tearing down the tender/pricing ERP I pruned exports from `packages/shared` and verified `@evertrust/api` + `@evertrust/db` + `@evertrust/shared` (build + 354 tests) — but NOT `@evertrust/web`. The web still imported the removed tender DTOs/types (`src/lib/api.ts`, `query-keys.ts`, `tender-format.ts`, `hooks/use-performance.ts`), so the next push failed Vercel deploy + the CI `verify` job. The user surfaced it as "vercel deployment error".
- **Lesson:** `packages/shared` is consumed by BOTH `erp-server` and `erp-client`. I scoped verification to the package I was actively editing (the API) and assumed the rest followed, so a cross-package break slipped through to CI/deploy.
- **Rule going forward:** Any change to `packages/shared` (or `packages/db`) is a cross-package change — before committing, run the workspace-wide gates the CI runs (`pnpm run typecheck && pnpm run lint && pnpm run test && pnpm run build`, which turbo fans out to web+api+db+shared), or at minimum `--filter @evertrust/web` AND `--filter @evertrust/api`. Never declare a shared-layer change green on one consumer's build alone.

### 2026-06-30 - A red `main` FREEZES Render + Vercel, and they don't auto-resume
- **Trigger:** Merged a reach feature PR whose change to `estimateScrapeSeconds` made an existing API test (`reach.scrape-async.spec.ts`, pinned to the OLD `460` value) fail — so `main`'s CI (`ci.yml`) went red. Render and Vercel both use native git integration **gated on CI**, so they stopped deploying and **froze at the last green commit** (the prior PR). A follow-up PR fixed the test and CI went green, but **neither platform auto-redeployed the skipped commits** — both stayed frozen until manually redeployed. Net effect: migrations + endpoint + FE all silently stale on prod for hours.
- **Lesson:** (1) I verified the feature with typecheck + lint + the agent's own suite but NOT the API jest suite — which (Docker was available) would have caught the pinned `460` assertion before merge. (2) A red `main` is not just hygiene — it is a **production deploy outage**: CI-gated platforms freeze and do not self-heal when CI recovers.
- **Rule going forward:** For any change to reach/ERP **service logic** (not just types), run the API jest suite locally before merging — `corepack pnpm --filter @evertrust/api test -- <spec>` (Docker must be up for the Testcontainer). When changing a formula/behavior, grep tests for assertions that pin the old value and update them in the SAME PR. And after ANY red-`main` incident, don't assume green CI re-deploys — **manually redeploy Render + Vercel** (Manual Deploy / Redeploy latest commit) and confirm the deployed SHA matches `main` HEAD; the mini agent never auto-deploys and always needs `git pull` + uvicorn restart.
