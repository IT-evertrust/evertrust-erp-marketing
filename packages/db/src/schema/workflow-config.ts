...
// GLOBAL, app-wide SINGLETON config for the Growth-Engine PLATFORM INFRASTRUCTURE.
// Lets admins override what is otherwise env-only (the n8n webhook URLs, the n8n
// API base, the ingest token, and the sequence offsets) from the ERP, without a
// redeploy. This config is GLOBAL — there is deliberately NO organizationId; there
// is ONE shared n8n + API for all tenants. A single row is enforced by `singleton`
// + its unique constraint (the find-or-create target).
//
// The CUSTOMER-FACING outreach PREFS (default_template, signature[+image], tone,
// template_language, default_sender, and the lead-governance caps/booleans) used to
// live here too; migration 0026 SPLIT them out into the PER-ORG `org_config` table
// (one row per org). Only platform wiring remains here now.
//
