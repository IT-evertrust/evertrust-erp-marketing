...
export const workflowConfig = pgTable(
  'workflow_config',
  {
    ...
    // NOTE: the Templates/Leads/sender governance fields (default_template, signature,
    // tone, template_language, max_*, daily_send_cap, default_regions,
    // respect_suppressions, dedup_days, require_niche_analysis, default_sender) were
    // MOVED to the per-org `org_config` table (see org-config.ts) in the canonical
    // migrations and no longer live here.
