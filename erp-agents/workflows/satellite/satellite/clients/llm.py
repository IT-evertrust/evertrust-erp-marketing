...
def classify_company(settings, name: str, url: str, text: str, niche: str, country: str = "",
    ...
    system = (
        ...
        "directory, training. 'company' = a real commercial business that SELLS its own "
        "products/services to other businesses (a B2B sales prospect). The others are NOT prospects: "
        "an event/conference page, an industry association / initiative / publicly-funded support "
        "project / alliance, a public authority, a university/school, a news/blog/magazine article, a "
        "job board or single job posting, a business directory / marketplace / comparison portal, or a "
        "training/course/certification provider — classify by what the PAGE is, in any language.\n"
        "nicheFit — is the NICHE the company's OWN core offering? Judge the ACTUAL business, not "
        "surface keywords. 'core' = selling the niche IS their main business. 'peripheral' = they "
        "genuinely sell it as one real secondary service among others. 'none' = a DIFFERENT field, "
        "even if the words sound related. Critically: a company in an ADJACENT-but-different domain "
        "is 'none' — e.g. for a CYBER/IT-security niche, a PHYSICAL-security firm (guards, alarms, "
        "CCTV/video surveillance, locks, building access control, fire alarms) is 'none'; a global "
        "software product, a generic web/marketing agency, or an insurance/consultancy that merely "
        "MENTIONS the niche is 'none' or at most 'peripheral', never 'core'."
    )
    user = (f'NICHE: "{niche}"\nCOUNTRY: {country}\nCOMPANY NAME: {name}\nURL: {url}\n'
            f'VISIBLE PAGE TEXT:\n{(text or "")[:4000]}\n\n'
