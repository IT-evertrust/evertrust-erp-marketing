...
def _resolve_persona(personas: list[dict], target_name: str) -> tuple[str, str]:
    ...
    if not match and personas:
        match = personas[0]
    if not match:
        # No persona store (e.g. local ERP without /personas) — fall back to a default
        # coaching prompt keyed on the requested name so the analysis still runs.
        name = target_name or "Sales Coach"
        return f"You are {name}, a sharp, candid B2B sales coach.", name
