...
class ErpClient:
    ...
    def get_personas(self, limit: int = 50) -> list[dict]:
        # Tolerate a missing endpoint (local ERP without /personas) — fall back to no stored
        # personas; the pipeline then uses the requested persona name's default prompt.
        try:
