...
def main() -> None:
    ...
    if len(sys.argv) != 2 or sys.argv[1] not in ("info", "hanna", "me"):
        raise SystemExit("Usage: python -m bazooka.auth <info|hanna|me>")
    account = sys.argv[1]
    ...
    # Pinned port so a Web-application OAuth client's registered redirect URI matches.
    # Register EXACTLY  http://localhost:54321/  (with trailing slash) in the client's
    # Authorized redirect URIs. Desktop-app clients ignore this and accept any port.
