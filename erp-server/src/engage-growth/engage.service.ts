...
// The agent is brain-only (engage.reply_glock / engage.rag_agent, registered on the
// monolith /run); sending/booking stay here.
@Injectable()
export class EngageService {
  ...
  async syncInbox(
    ...
    const accounts = await this.googleAccounts.listForOrg(orgId);
    let scanned = 0;
    let matched = 0;
    let ingested = 0;
